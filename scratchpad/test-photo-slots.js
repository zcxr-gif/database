...
const NAMES = ['MAX_AIRCRAFT_IMAGES', 'resolveAircraftSlot', 'applyAircraftPlacement'];
...
for (const action of ['insert', 'add', 'replace', null]) {
...
for (let count = 1; count <= 2; count++) {
    for (let slot = 1; slot <= count; slot++) {
        ...
        const { images } = run(before, 'insert', slot);
        const kept = before.every(u => images.includes(u));
        if (!kept) { failures++; console.log('  ✗', `insert at ${slot} on ${count} photo(s) dropped one`); }
    }
}
